...
class CBSignInStack extends Stack {
  /**
   *
  ...
  constructor(scope, id, props) {
    ...
    const CBSignInLambda = createLambdaFunction(
      this,
      "CBSignInLambda",
      Runtime.NODEJS_16_X,
      "CBSignIn.handler",
    ...
    const api = createApi(this, "CBSignInAPIGateway", "Endpoint for signIn lambda function");

    // Create the API Gateway resource
    const resource = createApiResource("cbsignin", "POST", CBSignInLambda, api);

    //Output the URLs for hitting the Lambda functions
    new CfnOutput(this, "CBSignInURL", {
      value: api.urlForPath(resource.path),
    });
  }
}

module.exports = { CBSignInStack };
