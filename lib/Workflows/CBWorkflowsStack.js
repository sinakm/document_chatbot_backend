...
const CBWorkflowsAdminStack = require("./CBWorkflowsAdminStack.js");

class CBWorkflowsStack extends Stack {
  /**
   *
  ...
  constructor(scope, id, props) {
    ...
    const CBWorkflowsLambda = createLambdaFunction(
      this,
      "CBWorkflowsLambda",
      Runtime.NODEJS_16_X,
      "CBWorkflows.handler",
    ...
    const api = createApi(this, "workflowsApiGateway", "Endpoint for basic lambda function");
    ...
    const resource = createApiResource("cbworkflows", "POST", CBWorkflowsLambda, api, "COGNITO_USER_POOLS", apiAuthorizer);

    new CBWorkflowsAdminStack.CBWorkflowsAdminStack(this, "CBWorkflowsAdminStack", props);

    //Output the URLs for hitting the Lambda functions
    new CfnOutput(this, "CBWorkflowsURL", {
      value: api.urlForPath(resource.path),
    });
  }
}

module.exports = { CBWorkflowsStack };
